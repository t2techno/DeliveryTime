...
type IDBEvent = Event & { target: IDBRequest | null };
...
// [[startTime, endTime]]
let contractionHistory: Array<Array<number>> = [];
...
const dbOpenSuccess = (event: IDBEvent) => {
  ...
  if (!event?.target === null) {
    console.error("db open event is success, but no database returned", event);
    return;
  }
...
const dbOnError = (db: IDBDatabase, event: IDBEvent) => {
  console.error(
    `Database error: ${
      event?.target !== null
        ? event?.target.error?.message
        : "No event targets, no error messages"
    }`,
  );
};

// initialize db from scratch
const dbOnUpgrade = (event: IDBEvent) => {
  // Save the IDBDatabase interface
  if (event?.target === null) {
    console.error("There was a problem upgrading the database");
    return;
  }
  ...
  createDbTx.transaction.oncomplete = () => {
...
const resetDbStores = (isReset?: boolean) => {
  // init settings store
...
const initSettingsStore = (isReset?: boolean) => {
...
// called in dbOnUpgrade and resetDbStores
// isReset makes the operation a put
const initEnergyStore = (isReset?: boolean) => {
...
// attach input listeners for key accessibility
const tabElements = document.getElementsByClassName("tab_able");
