...
const ContractionProvider: React.FC<PropsWithChildren> = ({children}) => {
    ...
    const [contractionHistory, setContractionHistory] = useState<ContractionItem[]>([]);
    ...
    const toggleContractions = useCallback((time: number)=>{
        ...
        setIsContracting(val => {
            ...
            if(!val){
                ...
                setContractionHistory(history => [...history, {startTime: contractionStartTime, endTime: time}])
    ...
    const valueOut = useMemo(() => {
        return ({hasStarted, isContracting, toggleContractions, numContractions, contractionStartTime});
